...
const Payment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValidEmail) {
      alert("❌ This email is not register or wrong!");
      return;
    }

    alert(
      `✅ Payment processed for ${email}\nMethod: ${paymentMethod}\nPackage: ${selectedPackage}\nTotal: $${total.toFixed(
        2
      )}`
    );
  };
        ...
        <div className="bg-white dark:bg-slate-900/70 backdrop-blur rounded-2xl shadow-xl ring-1 ring-black/5 dark:ring-white/10 overflow-hidden">
          ...
          <form className="p-6 space-y-5" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              />
              {isValidEmail === true && (
                <p className="text-green-600 text-sm">✅ Email found</p>
              )}
              {isValidEmail === false && (
                <p className="text-red-600 text-sm">❌ Email not registered</p>
              )}
            </div>
            <div className="space-y-2">
              <label
            ...
            <div className="flex items-center gap-3">
              <button
                type="submit"
                className="flex-1 inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-white font-medium shadow hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                Pay Now
              </button>
              <button
                type="button"
                className="rounded-xl px-4 py-2.5 font-medium text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/60"
              ><Link to="/">  Cancel</Link>
